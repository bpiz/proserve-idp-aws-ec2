 ...
 */
class Ec2Instance extends pulumi.ComponentResource {
    constructor(name, args, opts) {
        ...
        const rootBlockDevice = this.createRootBlockDevice(args.rootBlockDevice, baseTags);
        ...
        if (args.ebsVolumes) {
            args.ebsVolumes.forEach((volumeArgs, index) => {
                ...
                const volume = new aws.ebs.Volume(`${name}-volume-${volumeArgs.name}`, volumeArgsForVolume, { parent: this });
                // Attach the volume to the instance
                new aws.ec2.VolumeAttachment(`${name}-attachment-${volumeArgs.name}`, {
     ...
     */
    createRootBlockDevice(rootBlockDeviceArgs, tags) {
        const defaultConfig = {
            volumeSize: 8,
            volumeType: "gp3",
     ...
     */
    createCloudWatchAlarms(name, instance, args) {
        ...
        if (args.enableDetailedMonitoring !== false) {
            new aws.cloudwatch.MetricAlarm(`${name}-memory-alarm`, {
        ...
        // Status check alarm
        new aws.cloudwatch.MetricAlarm(`${name}-status-alarm`, {
    ...
    validateInstanceType(instanceType) {
        const instanceTypeRegex = /^[a-z0-9]+\.(nano|micro|small|medium|large|xlarge|2xlarge|4xlarge|8xlarge|16xlarge|32xlarge|metal)$/i;
    ...
    validateAmiId(amiId) {
        ...
        if (!amiIdRegex.test(amiId)) {
            throw new Error(`Invalid AMI ID format: ${amiId}. Expected format: ami-xxxxxxxx or ami-xxxxxxxxxxxxxxxxx`);
        }
    }
    /**
    ...
    validateSubnetId(subnetId) {
        ...
        if (!subnetIdRegex.test(subnetId)) {
            throw new Error(`Invalid subnet ID format: ${subnetId}. Expected format: subnet-xxxxxxxx or subnet-xxxxxxxxxxxxxxxxx`);
        }
    }
    /**
    ...
    validateSecurityGroupId(sgId, index) {
        ...
        if (!sgIdRegex.test(sgId)) {
            throw new Error(`Invalid security group ID format at index ${index}: ${sgId}. Expected format: sg-xxxxxxxx or sg-xxxxxxxxxxxxxxxxx`);
        }
    }
    /**
     * Validates EBS volume configuration
     */
    validateEbsVolume(volume, index) {
        if (!volume.name) {
            throw new Error(`EBS volume name is required at index ${index}`);
        }
        if (!volume.size || volume.size < 1 || volume.size > 16384) {
            throw new Error(`EBS volume size must be between 1 and 16384 GB at index ${index}`);
        }
        if (!volume.availabilityZone) {
            throw new Error(`EBS volume availability zone is required at index ${index}`);
        }
        const validTypes = ["standard", "gp2", "gp3", "io1", "io2"];
        if (volume.type && !validTypes.includes(volume.type)) {
            throw new Error(`Invalid EBS volume type at index ${index}: ${volume.type}. Valid types: ${validTypes.join(", ")}`);
        }
